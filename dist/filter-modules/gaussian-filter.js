export default (imageData, kernelSize, sigma) => {
        ...
        for (let x = padding; x < width - padding; x++) {
            ...
            for (let ky = -padding; ky <= padding; ky++) {
                for (let kx = -padding; kx <= padding; kx++) {
                    ...
                    const weight = kernel[(ky + padding) * kernelSize + (kx + padding)];
...
function makeGaussKernel(sigma, kernelSize) {
    ...
    for (let y = 0; y < kernelSize; y++) {
        for (let x = 0; x < kernelSize; x++) {
            ...
            const value = Math.exp(-(dx * dx + dy * dy) / (2 * s2)) / (2 * Math.PI * s2);
